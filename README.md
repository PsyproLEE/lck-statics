...
필터링해서 보는 Streamlit 대시보드입니다. 데이터 범위: **LCK 2015~현재**
...
브라우저에서 사이드바 필터로 년도·대회·시즌·라운드·팀·포지션을 조절하면 됩니다.
고급 필터에서 패치·챔피언 필터와 partial 데이터 제외 토글도 제공합니다.

- **선수 리더보드** — 필터 조건 내 선수별 집계(승률·KDA·DPM·GPM·CS/분·KP% 등), CSV 내려받기.
  DPM/GPM/CS·분당 지표는 경기 시간 가중(총량 ÷ 총 시간) 방식.
- **선수 상세** — 선수별 시즌 추이 그래프 + 챔피언 폭 (동명이인은 활동 연도로 구분 표시)
- **팀** — 팀별 승률 + 블루/레드 진영별 승률

`LCK 소속 팀만` 토글을 끄면 국제대회 상대 팀까지 포함됩니다.
켜진 상태에서 대회를 `MSI`/`WLDs`로 바꾸면 LCK 팀들의 국제대회 성적을 봅니다.

## 친구와 공유 (Streamlit Community Cloud · 무료 영구 링크)

앱이 쓰는 건 작은 parquet 2개(약 10MB)뿐 — 원본 779MB·gol.gg 캐시는
`.gitignore`로 제외되고 **코드 + 작은 parquet만** 올라갑니다.

1. GitHub에 **공개 저장소** 생성 후 푸시:

   ```powershell
   git commit -m "LCK stats dashboard"
   git remote add origin https://github.com/<사용자명>/lck-statics.git
   git push -u origin main
   ```

2. <https://share.streamlit.io> → GitHub 로그인 → **New app**
   - Repository: `<사용자명>/lck-statics`
   - Branch: `main`
   - **Main file path: `app/dashboard.py`**
   - (선택) Advanced → Python 3.12
3. **Deploy** → 몇 분 후 `https://<...>.streamlit.app` 영구 링크 생성.
   친구는 이 링크만 있으면 접속(내 PC 꺼져 있어도 됨). 유휴 시 잠자다
   접속하면 깨어남.

> 데이터가 공개 저장소에 올라갑니다. 개인·비상업 + 출처 표기(앱 하단·README)
...
2. `python src/scrape_golgg.py --no-cache` — gol.gg 솔킬 재수집
   (실패해도 기존 parquet 유지)
3. parquet이 바뀌었을 때만 자동 커밋·푸시 → Streamlit Cloud 자동 재배포

수동 실행: GitHub 저장소 → Actions → *Update data* → Run workflow.
로컬 수동 갱신도 동일: `py src/update.py` + `py src/scrape_golgg.py` 후
parquet을 커밋·푸시하면 됩니다.

## 구조

```
data/raw/             OE 원본(.csv/.xlsx) + golgg_cache/ — git 제외
data/processed/       lck.parquet, solokills.parquet (생성물·소형, 배포용 커밋)
src/ingest.py         OE 원본 → 정제 parquet (playerid 보존, 2015+ 필터)
src/update.py         OE Drive에서 연도별 CSV 다운로드 + 증분 병합
src/scrape_golgg.py   gol.gg 솔킬 스크랩 (토너먼트 자동 발견) → solokills.parquet
src/queries.py        집계/필터 + 솔킬 조인 (Streamlit 비의존)
app/dashboard.py      Streamlit UI
src/golgg_name_overrides.json  (선택) gol.gg→OE 선수명 보정 맵
tests/                pytest 단위 테스트 (py -m pytest)
.github/workflows/update-data.yml  매일 자동 데이터 갱신
```
