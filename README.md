...
> - gol.gg는 솔킬 0인 선수를 대회 페이지에서 생략 → 일부 주전 공란 가능하고,
>   gol.gg의 경기수 합은 분모로 못 씀(과소집계). **솔킬/G의 분모는 OE 기준
>   실제 출전 경기수**로 계산 (2026-07 교차검증에서 발견·수정)
...
- **선수 리더보드** — 선수별 집계(승률·KDA·DPM·GPM·CS/분·KP% 등), 컬럼 클릭
  ...
  기본은 핵심 지표 8개만 표시(전체 지표 보기 버튼), 각 셀의 금색 바는 현재
  필터 내 상대 순위(퍼센타일). 상단 프리셋 칩(올해 시즌·역대 통산·국제대회)으로
  원클릭 이동.
- **선수 상세** — 프로필 헤더(팀·포지션·요약 칩) + 시즌 추이 그래프 +
  시즌별/챔피언별 표 (더블클릭으로 이동, 동명이인은 활동 연도로 구분)
- **선수 비교** — 최대 3명: 6축 레이더(같은 포지션 내 백분위), 연도별 추이
  중첩 그래프, 지표별 비교 표(최고값 하이라이트)
- **팀** — 팀 승률 + 블루/레드 진영별 승률 (포지션·챔피언 필터는 팀 탭에 미적용).
  팀을 클릭하면 **시즌별 로스터**(포지션별 주전·서브 + 경기수, 선수 클릭 시 상세
  이동). 리더보드의 팀 표시는 **최근 소속팀** 기준이며 이적 이력은 "외 N팀"
  툴팁으로 표시
...
### 데이터 정확성 (교차검증 결과, 2026-07)

`py src/validate.py`(+`--leaguepedia`)로 재실행 가능한 3축 검증:

- **내부**: 경기당 선수 행 10개(예외 10경기 — OE 원본 결측, 주로 2021 T1),
  중복 없음, 연도별 경기 수 정상 범위
- **vs gol.gg**: LCK Spring 2024 — 61명 전원 경기수·KDA 일치.
  LCK Summer 2023 — 66명 중 65명 일치(Gumayusi 1경기 차이, 리메이크 추정)
- **vs Leaguepedia**: 2023 Summer 정규 206·플옵 31, 2024 Spring 210 등
  정확 일치. 2016·2019의 차이는 **OE가 승강전을 다음 스플릿 정규에 포함**하기
  때문(분류 방식 차이, 오류 아님) — 해당 경기들은 Leaguepedia 승강전 페이지와
  팀·날짜 대조로 식별해 라운드 **"승강전"**으로 재분류했으며
  (`src/promotion_gameids.json`), 라운드 필터에서 제외/포함을 선택할 수 있음
